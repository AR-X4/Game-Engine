...
namespace Azul
{
	...
	void TextureManager::Add(const char* const _assetName, unsigned char* textBuffer, const Texture::Name _name)
	{
		GLuint               textureID;
		GLuint* pTextureID = &textureID;
		...
		int width;
		int height;
		pTextMan->privLoadTexture(_assetName, textBuffer, pTextureID, width, height);
	...
	void TextureManager::privLoadTexture(const char* const _assetName, unsigned char* textBuffer, GLuint*& textureID, int& width, int& height)
	{
		// Get an ID, for textures (OpenGL poor man's handle)
		glGenTextures(1, textureID);
		...
		// Loat the texture
		this->privLoadTGATexture(_assetName, textBuffer, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, width, height);
	...
	bool TextureManager::privLoadTGATexture(const char* szFileName, unsigned char* textBuffer, GLint minFilter, GLint magFilter, GLint horizWrapMode, GLint vertWrapMode, int& nWidth, int& nHeight)
	{
		GLbyte* pBits;
		int nComponents;
		GLenum eFormat;
